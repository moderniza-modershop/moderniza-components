...
const View = forwardRef((props, ref) => {
   ...
   */
  const options = props.options.build
    ? props.options
    : new DataviewOptions(props.options)
  ...
  const [localLayout, setLocalLayout] = useState(
    localStorage.getItem(
      btoa(`${window.location.pathname}|mdz-dataview-layout`)
    )
  )
  ...
  useEffect(() => {
    if (
      typeof lastDeviceSize.width === "number" &&
  ...
  useEffect(() => {
    if (
      localLayout &&
      supportedLayouts.includes(localLayout) &&
      Object.keys(templates)
        .map((layout) => {
          return layout === "columns" ? "table" : layout
        })
        .includes(localLayout)
        ...
        >
          {options.expand ? (
            <Column
              ...
              style={{ width: "25px" } && (options.expand.style || {})}
            />
          ) : null}
