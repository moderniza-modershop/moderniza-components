 ...
 */
export const supportedLayouts = ['grid', 'list', 'table']

/**
 * Supported Dataview export extensions
 */
export const supportedExtensions = ['csv', 'xlsx', 'pdf']
...
class DataviewOptions {
  /**
     ...
     * @param {{header: String, headerStyle: Object, headerClassName: String, field: String, sortable: Boolean, sortField: String, filter: Boolean, filterField: String, filterElement: Function, filterFunction: function, showFilterOperator: Boolean, showFilterMatchModes: Boolean, showAddButton: Boolean, body: Function, bodyStyle: Object, bodyClassName: String, frozen: Boolean, style: Object, exportable: Boolean}[]} options.templates.columns columns template
     ...
     * 
     * @param {Function} options.expand Expander options
     * @param {Function} options.expand.frozen Expander frozen
     * @param {Function} options.expand.expander Expander function
     * @param {object} options.expand.style Expander style
     *
  ...
  defaults(options) {
    ...
    options.templates.columns = options.templates.columns.map((col) => {
      return {
        ...col,
        exportable: col.exportable === false || col.exportable === true ? col.exportable : defaultColumn.exportable,
        filter: col.filter === false || col.filter === true ? col.filter : defaultColumn.filter,
        sortable: col.sortable === false || col.sortable === true ? col.sortable : defaultColumn.sortable
      }
    })
    ...
    if (options.sorts) {
      ...
      if (options.sorts.visible !== false) {
    ...
    if (options.pagination) {
      ...
      if (options.pagination.visible !== false) {
    ...
    if (options.export) {
      ...
      if (options.export.visible !== false) {
